...
def train(classifier, inversion, log_interval, device, data_loader, optimizer, epoch, penalty, norm_l, path_out, classifier_sub, lambda_adv, adv_param, logger, k_top, train_or_not):
    classifier.eval()
    classifier_sub.eval()
    inversion.train()
    # com_suc = 1
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        if penalty == 'no':
            ...
            loss = F.mse_loss(reconstruction, data) + lambda_adv*F.cross_entropy(classifier_sub(reconstruction, celoss=True), classifier_sub(data, release=True))
        loss.backward()
        optimizer.step()
        ...
        for norm in norm_list:
            ...
            if penalty == 'no':
                ...
                loss = F.mse_loss(reconstruction_adv, data_adv) + lambda_adv*F.cross_entropy(classifier_sub(reconstruction_adv, celoss=True), classifier_sub(data_adv, release=True))
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        train(classifier, inversion, args.log_interval, device, train_loader, optimizer, epoch, args.penalty, args.norm, 'Inversion_Models/'+args.path_out, classifier_sub, args.lambda_pen, args.adv_param, logger, args.k_top, args.train_or_not)
        ...
        if recon_loss < best_recon_loss:
            ...
            shutil.copyfile('Inversion_Models/'+args.path_out + 'recon_round_attack.png', 'Inversion_Models/' + args.path_out + 'best.png')
